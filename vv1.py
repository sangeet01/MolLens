...
# Subsample for SSL pretraining (30% of training data)
df_train, df_val = train_test_split(df, test_size=0.1, random_state=42)
df_ssl = df_train.sample(frac=0.3, random_state=42)

# SMILES Tokenization
all_smiles = df_train['smiles'].tolist()
unique_chars = set(''.join(all_smiles)) | {MASK_TOKEN}
valid_atoms = {'C', 'N', 'O', 'S', 'P', 'F', 'Cl', 'Br', 'I', 'H'}  # Common atoms for SMILES
tokens = [PAD_TOKEN, SOS_TOKEN, EOS_TOKEN, MASK_TOKEN] + sorted(unique_chars - {PAD_TOKEN, SOS_TOKEN, EOS_TOKEN, MASK_TOKEN})
...
# Precompute Morgan fingerprints for validation SMILES
val_smiles = df_val['smiles'].tolist()
val_fingerprints = {}
morgan_gen = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=2048)
for smiles in val_smiles:
    mol = Chem.MolFromSmiles(smiles)
    if mol:
        val_fingerprints[smiles] = morgan_gen.GetFingerprint(mol)
...
def supervised_train(model, train_loader, val_loader, epochs=30, lr=1e-4, patience=5):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{ssl_pretrain} - Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, LR: {optimizer.param_groups[0]['lr']:.6f}")
        scheduler.step(avg_val_loss)
...
# Beam Search with SMILES Constraints
def beam_search(model, spectrum, ion_mode_idx, precursor_idx, true_smiles, beam_width=10, max_len=150, nucleus_p=0.9, device='cpu'):
    model.eval()
    true_mol = Chem.MolFromSmiles(true_smiles) if true_smiles else None
    with torch.no_grad():
        spectrum = spectrum.unsqueeze(0).to(device)
...
# Tanimoto Similarity with Precomputed Fingerprints
...
for sample_idx in range(num_samples):
    ...
    predicted_results = beam_search(model, sample_spectrum, sample_ion_mode, sample_precursor_bin, true_smiles, beam_width=10, max_len=SUPERVISED_MAX_LEN, device=device)
    print(f"\nSample {sample_idx} - True SMILES: {true_smiles}")
    print("Top Predicted SMILES:")
    for smiles, confidence in predicted_results[:3]:
        similarity = tanimoto_similarity(smiles, true_smiles, precomputed_fps=val_fingerprints)
        print(f"SMILES: {smiles}, Confidence: {confidence:.4f}, Tanimoto: {similarity:.4f}")
        if len(smiles) > 100 and smiles.count('C') > len(smiles) * 0.8:
            print("Warning: Predicted SMILES is a long carbon chain, indicating potential model underfitting.")
        if smiles != "Invalid SMILES":
    ...
    if predicted_results[0][0] != "Invalid SMILES":
        ...
        if pred_mol and true_mol:
            ...
            plt.imshow(img_array)
            plt.axis('off')
            plt.title(f"Sample {sample_idx} - Tanimoto: {tanimoto_similarity(predicted_results[0][0], true_smiles, val_fingerprints):.4f}")
            plt.show()
